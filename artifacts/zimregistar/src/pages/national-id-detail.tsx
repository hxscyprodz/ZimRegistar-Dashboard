...
import { ChevronLeft, CheckCircle, XCircle, Printer, Clock } from "lucide-react";
...
export function NationalIdDetailPage() {
  const { id } = useParams();
  const [, navigate] = useLocation();
  const { user } = useAuth();
  const stationId = useUserStation();
  const { nationalId, approve, reject, markPrinted } = useApps();
  const rawApp = nationalId.find(a => a.id === id);
  ...
  const handleApprove = () => {
    approve("nationalId", app.id, user?.name ?? "Staff");
    toast.success("Application approved successfully.");
  };

  const handleReject = (reason: string) => {
    reject("nationalId", app.id, reason, user?.name ?? "Staff");
    toast.error("Application rejected.");
  };

  const handlePrint = () => {
    if (app.printStatus !== "Printed") markPrinted("nationalId", app.id);
    window.print();
  };
  ...
  return (
    <div className="space-y-6 pb-10 print-document-host">
      <div className="no-print">
        <Link href="/applications/national-id" className="inline-flex items-center text-xs font-medium text-muted-foreground hover:text-foreground mb-4 transition-colors">
        ...
        <div className="lg:col-span-2 space-y-6">
          <DataCard title="Applicant Information">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2">
              ...
              <InfoRow label="Date of Birth" value={format(new Date(app.applicant.dateOfBirth), "MMMM d, yyyy")} />
          ...
          <DataCard title="Biometrics & Documents">
            <div className="grid grid-cols-2 gap-4">
              <DocumentCard label="ID Photo" src={app.documents.photo} />
            ...
            {app.status === "Pending" ? (
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2 border-t border-border">
                  ...
                  <Button variant="outline" className="w-full" onClick={() => setPrintOpen(true)}><Printer className="mr-2 h-4 w-4" /> Print ID Card</Button>
                </div>
              </div>
            ) : (
      ...
      <ConfirmModal open={approveOpen} onOpenChange={setApproveOpen} title="Approve Application" description="Confirm application approval for printing." confirmLabel="Approve" tone="success" onConfirm={handleApprove} />
      ...
      <Dialog open={printOpen} onOpenChange={setPrintOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0 no-print">
          <div className="bg-muted px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <Button onClick={handlePrint} size="sm"><Printer className="mr-2 h-4 w-4" /> Print ID Card</Button>
          </div>
