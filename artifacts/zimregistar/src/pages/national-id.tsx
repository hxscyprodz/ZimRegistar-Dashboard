...
export function NationalIdListPage() {
  const stationId = useUserStation();
  const rawApps = useApps(s => s.nationalId);
  ...
  const filtered = apps.filter(a => {
    ...
    if (search) {
      ...
      return a.applicationNumber.toLowerCase().includes(q) ||
             ...
             a.applicant.contactNumber.includes(q);
    }
    return true;
  }).sort((a, b) => {
  ...
  return (
    <div className="space-y-4 pb-10">
      <PageHeader 
        ...
        description="Manage and process first-time national ID registrations."
      ...
      <TableContainer>
        ...
        <tbody className="divide-y divide-border/60">
          {paginated.length === 0 ? (
            <tr><Td className="py-12 text-center" colSpan={8}><EmptyState icon={IdCard} title="No applications found" /></Td></tr>
          ) : paginated.map((app) => (
            <tr key={app.id} className="hover:bg-muted/30 transition-colors">
              ...
              <Td>{format(new Date(app.applicant.dateOfBirth), "dd MMM yyyy")}</Td>
              <Td>{app.applicant.gender}</Td>
              <Td className="max-w-[150px] truncate" title={app.applicant.address}>{app.applicant.address}</Td>
              <Td><span className="text-muted-foreground">{format(new Date(app.dateSubmitted), "dd MMM yyyy")}</span></Td>
              <Td><StatusBadge status={app.status} /></Td>
              <Td className="text-right">
                <Link href={`/applications/national-id/${app.id}`} className="text-xs font-medium text-primary hover:underline">
                  View Details
                </Link>
              </Td>
            </tr>
